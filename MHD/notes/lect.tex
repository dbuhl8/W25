...
\begin{gather*}
    ...
    \F\cdot\uved dV + \int_{\partial D} \bs{\tau}\cdot\uvec dS - \int_{\partial
    D} \bs{q}\cdot dS \\
...
\begin{gather*}
    \rho\DDt{e} = -\grad \cdot \bs{q} - p\left(\grad\cdot\uvec) + \Phi
\end{gather*}

The Second law of Thermodynamics also plays an important role in the
conservation of energy. It states that the entropy of a system $S$
\begin{gather*}
    dS = \frac{dq}{T}\\
    TdS =  de + pdV\\
...
\begin{align*}
    \grad\cdot\bvec =& 0 & \grad \cdot \bs{E} = \frac{\rho_e}{\varepsilon_0}\\
...
\begin{align*}
    &\oint_{\partial D} \bvec\cdot\bs{dA} = 0 & \oint_{\partial D}
    \bs{E}\cdot\bs{dA} = \frac{q}{\varepsilon_0}\\
    &\oint_L \bs{E}\cdot\bs{dL} = -\ppt{\phi_{B}} & \oint_L\bvec\cdot\bs{dL} =
    \mu_0\bs{i} + \mu_0\varepsilon_0\ppt{\phi_E}
\end{align*}
...
\begin{gather*}
    ...
    \phi_E = \int_S\frac{q_1}{4\pi\varepsilon r^2}\cdot\bs{dA}\\
    \phi_E = \frac{q_1}{4\pi\varepsilon r^2}\int_S\bs{dA}\\
    \phi_E = \frac{q_e}{\varepsilon}
\end{gather*} 
where $q_e$ in the final equation is given by the sum of all point charges
enclosed in the closed volume, i.e. $q_e = \sum_i q_i$. 
